...
import com.solvd.CarService.threads.MyRunnableThread;
import com.solvd.CarService.threads.MyThread;
...
//        pool.shutdown();
        ...
        //Testing Future/CompletionStage
        ...
        for (int i = 0; i < 7; i++){
            ...
            CompletableFuture<Void> stage = CompletableFuture
                    ...
                    .thenAccept(conn -> {
                        try {
                            String name = Thread.currentThread().getName();
                            System.out.println(name + "-> got" + conn);
                            Thread.sleep(2_000);
                            pool2.release(conn);
                            System.out.println(name + "-> release connection " + conn);
                        } catch (InterruptedException e) {
                            throw new RuntimeException(e);
                        }
                    });
